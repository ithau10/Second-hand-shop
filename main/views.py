...
def coolstuff(request):
	return render(request,'user/cool_stuff.html')
#randomFeature
def randomfeature(request):
	return render(request,'user/random_feature.html')
#teamfeature
def teamfeature(request):
	return render(request,'user/team_feature.html')

def developerstuff(request):
	return render(request,'user/developer_stuff.html')
def aboutGT(request):
	return render(request,'user/aboutGT.html')
    ...
    if 'cartdata' in request.session:
        ...
        for p_id, item in request.session['cartdata'].items():
            ...
            items = CartOrderItems.objects.create(
                order=order,
                product=product,
                invoice_no='INV2023-'+str(order.id),
                item=item['title'],
                image=item['image'],
                qty=item['qty'],
                price=item['price'],
        ...
        request.session.pop('cartdata', None)
